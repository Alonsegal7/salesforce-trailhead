...
import getTopParent from "@salesforce/apex/HierarchyController.getTopParent";
  ...
  fetchRecords(parentId, onLoad) {
    getRecordData({
      ...
      .then((data) => {
        ...
        //
        if (onLoad) {
            getTopParent({ soql: this.soql, recordId: parentId})
            .then((data) => {
                if (data) {
                    if (this.gridData != null && this.gridData.length > 0) {
                        let childrenArry = this.gridData;
                        let topParentObj = data.record;
                    topParentObj["_children"] = childrenArry;
                    let newGridData = [];
                    newGridData.push(topParentObj);
        ...
        //handle top parent
        //get the parent object - return a wrapperData
        //if this.gridData is not empty - set gridData to list of sigle element (top parent) with children = this.gridData
        //else set grid data to the return wrapperData
      })
      .catch((error) => {
        if (error) {
